...
If the upload picker is disabled, select **Check photo upload setup**. This authenticated server check reports missing setting names, an invalid cloud name, or surrounding whitespace in credentials under **Setup details for your website developer**. It never returns credential values. A successful recheck enables the picker without discarding form edits. Cloud names tolerate surrounding whitespace. This checks configuration presence/format only; it does not verify credentials with Cloudinary or prove a real upload will succeed. Deploy these changes before using the diagnostic on the hosted site.
